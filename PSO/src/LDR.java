...
public class LDR {
	...
	public static double[] ldr(movmain instance, double [] solucion, double fitnessSol) {
		double auxFitness = -1;
		boolean existeMejora = true; //número de iteracione sin mejora.
		boolean mejorado = false;
		//El siguiente array va a conener todas las posibles soluciones de la vecindad para ello se podrá realizar una
		//variación (delta) la cual se podrá aplicar a cada coordenada de manera positiva o negativa.
	...
	/**
	 * Este método será el encargado de generar todas las posibles soluciones cercanas a una dada como parámetro.
	 * A este conjunto de posibles soluciones se le denomina vecindad.
	 * Para realizar el cálculo de la misma realizaremos una alteración de la solución recibida, aumentando o disminuyendo 
	 * las coordenadas en un pequeño factor (delta).
	 * 
	 * Para realizar esta alteración, generamos todas las posibles permutaciones con todos los números binarios desde 0 hasta 2^n 
	 * siendo n el tamaño de la solución.
	 * 
	 * Despúes le aplicamos el pequeño empujón a la solucion, primero empujando y después restando.
	 * 
	 * Notas: Al solo combinar sumas por un lado y restas por otro me falta combinar las restas con las sumas.
	 * 
	 * @param solucion
	 * @return ArrayList <double[]> cada posición contiene una posible solución pertenenciente a la vecindad.  
	 */
	...
	private static ArrayList<double[]> generaVecindad(double [] solucion) {
		...
		for(int i = 1; i < numVariaciones;i++){ // la i empieza en 1 porque el 0 no altera las coordenadas.
			...
			for(int j = binario.length()-1; j >= 0; j--){
				...
				int valor = binario.charAt(binario.length() - j - 1) - 48; // Pasamos el valor a entero restándole el valor ASCII del 0
	...
	private static ArrayList<double[]> generaVecindadTotal(double [] solucion) {
		...
		//System.out.println("tamaño: "+vecindades.size());
	...
	public static void main(String args[]){
		...
		/*
		 * PRUEBA GENERACIÓN DE VECINDADES 
		 */
